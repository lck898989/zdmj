...
Page({
    ...
    acticleScene: function (event) {
        ...
        wx.navigateTo({
            url: '../../ActicleScene/ActicleScene?title=' + event.currentTarget.dataset.title + "&authorurl=" + event.currentTarget.dataset.authorurl + "&authorname=" + event.currentTarget.dataset.authorname + "&shopurl=" + event.currentTarget.dataset.shopurl + "&pid=" + event.currentTarget.dataset.pid,
        })
    },
    /**
     * 生命周期函数--监听页面初次渲染完成
    ...
    onReachBottom: async function () {
        if (this.data.choosed == false) {
            ...
            this.data.page++;
            //向服务器请求更多数据
            let req = new Request(app.host + 'Data/GetProducts', {
                page: this.data.page
            }, "POST", "text");
            console.log("ae32333");
            let res = await req.sendRequest();
            console.log("res.data.products is ", res.data.products);
